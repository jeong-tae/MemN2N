    ...
    def qa_model(self, task_id):
        ...
        if self.PE:
            ...
            for j in range(1, self.max_words+1):
                for k in range(1, self.edim+1):
                    pos_emb[j-1, k-1] = (1.-j/float(self.max_words))-(k/float(self.edim))*(1.-2*j/float(self.max_words))

            self.pos_emb = tf.reshape(tf.stack([pos_emb]*self.batch_size*self.max_mem_size), [self.batch_size, self.max_mem_size, self.max_words, self.edim])
        ...
        for h in xrange(1, self.nhops+1):
            ...
            if self.weight_tying == "Adj" and h < self.nhops:
                emb_A = self.get_variable("C%d"%h, shape = None, reuse = True)
                ...
                if self.TE:
                    emb_T_A = self.get_variable("T_C%d"%h, shape = None, reuse = True)
        ...
        # training recommenced isn't applied in this code
    ...
    def train(self):
        ...
        for epoch in pbar1:
            ...
            for step in pbar2:
                ...
                pbar2.set_postfix(loss = loss, train_acc = "%2.f%%"%acc)

            test_acc, test_loss = self.test()
            pbar1.set_postfix(loss = test_loss, test_acc = "%2.f%%"%test_acc)
            if old_loss > test_loss: 
                # no validation data, so we use test data as validation
                self.saver.save(self.sess,
                        os.path.join("checkpoints/", "memN2N_task%d.model"%self.task_id),
                        global_step = epoch)
                old_loss = test_loss
                counting = 0
            else:
